...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (fetching) return <div style={{ padding: '48px', color: 'var(--color-text-muted)' }}>Loading...</div>;

  return (
    <div style={{ maxWidth: '640px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '24px', letterSpacing: '-0.02em' }}>Edit Product</h1>
      ...
      <form onSubmit={handleSubmit} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '8px', padding: '24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
          <div>
            ...
            <input value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value.toUpperCase() })} className="mono" />
          </div>
          <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
          ...
          <div>
            ...
            <input value={DIMENSION_LABELS[form.dimension]} disabled style={{ opacity: 0.5, cursor: 'not-allowed' }} />
          </div>
        </div>

        <div style={{ marginBottom: '16px', padding: '12px', background: 'var(--color-bg)', borderRadius: '6px' }}>
          <span style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>Base Unit (locked): </span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div>
            ...
            <input type="number" step="any" min="0" value={form.stockQuantity} onChange={(e) => setForm({ ...form, stockQuantity: e.target.value })} />
          </div>
          <div>
            ...
            <input type="number" step="any" min="0.00000001" value={form.basePricePerUnit} onChange={(e) => setForm({ ...form, basePricePerUnit: e.target.value })} />
          </div>
        </div>
